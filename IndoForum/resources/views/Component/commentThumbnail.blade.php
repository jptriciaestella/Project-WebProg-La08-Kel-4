        ...
        @if ($comment->user_id == Auth::user()->id || Auth::user()->role == 'admin')
            <div class="position-absolute bottom-0 end-0" style="margin: 1rem 2rem">
                @if ($comment->user_id == Auth::user()->id)
                    {{-- GANTI HREF --}}
                    <a href="/edit-comment/{{Auth::user()->id}}" class="btn btn-outline-success rounded-circle" style="margin-right: 10px">
                ...
                {{-- GANTI HREF --}}
                <a href="/delete-comment/{{Auth::user()->id}}" class="btn btn-outline-danger rounded-circle">
